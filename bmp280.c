...
static bmp280_err_t bmp280_bus_initialize(bmp280_t const *bmp280) {
  return bmp280->interface.bus_initialize
             ? bmp280->interface.bus_initialize(bmp280->interface.bus_user)
             : BMP280_ERR_NULL;
}

static bmp280_err_t bmp280_bus_deinitialize(bmp280_t const *bmp280) {
  return bmp280->interface.bus_deinitialize
             ? bmp280->interface.bus_deinitialize(bmp280->interface.bus_user)
             : BMP280_ERR_NULL;
}

static bmp280_err_t bmp280_bus_write_data(bmp280_t const *bmp280,
                                          uint8_t address, uint8_t const *data,
                                          size_t data_size) {
  return bmp280->interface.bus_write_data
             ? bmp280->interface.bus_write_data(bmp280->interface.bus_user,
                                                address, data, data_size)
             : BMP280_ERR_NULL;
}

static bmp280_err_t bmp280_bus_read_data(bmp280_t const *bmp280,
                                         uint8_t address, uint8_t *data,
                                         size_t data_size) {
  return bmp280->interface.bus_read_data
             ? bmp280->interface.bus_read_data(bmp280->interface.bus_user,
                                               address, data, data_size)
             : BMP280_ERR_NULL;
}

bmp280_err_t bmp280_initialize(bmp280_t *bmp280, bmp280_config_t const *config,
  ...
  return bmp280_bus_initialize(bmp280);
}

bmp280_err_t bmp280_deinitialize(bmp280_t *bmp280) {
  assert(bmp280);

  bmp280_err_t err = bmp280_bus_deinitialize(bmp280);
...
bmp280_err_t bmp280_get_press_data(bmp280_t *bmp280, int32_t *data) {
  ...
  var1 =
      (((((int64_t)1) << 47) + var1)) * ((int64_t)bmp280->trim_data.p1) >> 33;
...
bmp280_err_t bmp280_get_trim_data(bmp280_t const *bmp280,
                                  bmp280_trim_data_t *trim_data) {
  assert(bmp280 && trim_data);

  uint8_t data[24] = {};

  bmp280_err_t err = bmp280_bus_read_data(bmp280, BMP280_REG_ADDRESS_CALIB00,
                                          data, sizeof(data));
...
bmp280_err_t bmp280_get_calib_reg(bmp280_t const *bmp280, uint8_t num,
                                  bmp280_calib_reg_t *reg) {
  assert(bmp280 && reg);

  uint8_t data[2] = {};

  bmp280_err_t err = bmp280_bus_read_data(
      bmp280, BMP280_REG_ADDRESS_CALIB00 + 2 * num, data, sizeof(data));

  reg->calibration_data =
      (uint16_t)(((data[1] & 0xFFU) << 8U) | (data[0] & 0xFFU));

  return err;
}

bmp280_err_t bmp280_set_calib_reg(bmp280_t const *bmp280, uint8_t num,
  ...
  return bmp280_bus_write_data(bmp280, BMP280_REG_ADDRESS_CALIB00 + num * 2,
                               data, sizeof(data));
}

bmp280_err_t bmp280_get_reset_reg(bmp280_t const *bmp280,
                                  bmp280_reset_reg_t *reg) {
  assert(bmp280 && reg);

  uint8_t data = {};

  bmp280_err_t err = bmp280_bus_read_data(bmp280, BMP280_REG_ADDRESS_RESET,
                                          &data, sizeof(data));
...
bmp280_err_t bmp280_set_reset_reg(bmp280_t const *bmp280,
                                  bmp280_reset_reg_t const *reg) {
  assert(bmp280 && reg);
  ...
  return bmp280_bus_write_data(bmp280, BMP280_REG_ADDRESS_RESET, &data,
                               sizeof(data));
}

bmp280_err_t bmp280_get_config_reg(bmp280_t const *bmp280,
                                   bmp280_config_reg_t *reg) {
  assert(bmp280 && reg);

  uint8_t data = {};

  bmp280_err_t err = bmp280_bus_read_data(bmp280, BMP280_REG_ADDRESS_CONFIG,
                                          &data, sizeof(data));
...
bmp280_err_t bmp280_set_config_reg(bmp280_t const *bmp280,
                                   bmp280_config_reg_t const *reg) {
  assert(bmp280 && reg);
  ...
  err |= bmp280_bus_write_data(bmp280, BMP280_REG_ADDRESS_CONFIG, &data,
                               sizeof(data));

  return err;
}

bmp280_err_t bmp280_get_ctrl_meas_reg(bmp280_t const *bmp280,
                                      bmp280_ctrl_meas_reg_t *reg) {
  assert(bmp280 && reg);

  uint8_t data = {};

  bmp280_err_t err = bmp280_bus_read_data(bmp280, BMP280_REG_ADDRESS_CTRL_MEAS,
                                          &data, sizeof(data));
...
bmp280_err_t bmp280_set_ctrl_meas_reg(bmp280_t const *bmp280,
                                      bmp280_ctrl_meas_reg_t const *reg) {
  assert(bmp280 && reg);
  ...
  err |= bmp280_bus_write_data(bmp280, BMP280_REG_ADDRESS_CTRL_MEAS, &data,
                               sizeof(data));
...
bmp280_err_t bmp280_get_id_reg(bmp280_t const *bmp280, bmp280_id_reg_t *reg) {
  ...
  bmp280_err_t err =
      bmp280_bus_read_data(bmp280, BMP280_REG_ADDRESS_ID, &data, sizeof(data));
...
bmp280_err_t bmp280_get_status_reg(bmp280_t const *bmp280,
                                   bmp280_status_reg_t *reg) {
  assert(bmp280 && reg);

  uint8_t data = {};

  bmp280_err_t err = bmp280_bus_read_data(bmp280, BMP280_REG_ADDRESS_STATUS,
                                          &data, sizeof(data));
...
bmp280_err_t bmp280_get_press_reg(bmp280_t const *bmp280,
                                  bmp280_press_reg_t *reg) {
  assert(bmp280 && reg);

  uint8_t data[2] = {};

  bmp280_err_t err = bmp280_bus_read_data(bmp280, BMP280_REG_ADDRESS_PRESS_MSB,
                                          data, sizeof(data));
...
bmp280_err_t bmp280_get_press_x_reg(bmp280_t const *bmp280,
                                    bmp280_press_x_reg_t *reg) {
  assert(bmp280 && reg);

  uint8_t data = {};

  bmp280_err_t err = bmp280_bus_read_data(bmp280, BMP280_REG_ADDRESS_PRESS_XLSB,
                                          &data, sizeof(data));
...
bmp280_err_t bmp280_get_temp_reg(bmp280_t const *bmp280,
                                 bmp280_temp_reg_t *reg) {
  assert(bmp280 && reg);

  uint8_t data[2] = {};

  bmp280_err_t err = bmp280_bus_read_data(bmp280, BMP280_REG_ADDRESS_TEMP_MSB,
                                          data, sizeof(data));
...
bmp280_err_t bmp280_get_temp_x_reg(bmp280_t const *bmp280,
  ...
  bmp280_err_t err = bmp280_bus_read_data(bmp280, BMP280_REG_ADDRESS_TEMP_XLSB,
                                          &data, sizeof(data));
